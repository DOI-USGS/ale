...
class IsisSpice(IsisLabel):
    ...
    def _sensor_position(self):
        ...
        inst_positions_times = cube._inst_position_table["Times"]
        inst_positions_times = np.linspace(inst_positions_times[0], inst_positions_times[-1], self.number_of_ephemerides)

        # interpolate out positions
        f_positions_x = interp1d(cube._inst_position_table["Times"], cube._inst_position_table["Positions"].T[0])
        f_positions_y = interp1d(cube._inst_position_table["Times"], cube._inst_position_table["Positions"].T[1])
        f_positions_z = interp1d(cube._inst_position_table["Times"], cube._inst_position_table["Positions"].T[2])

        positions = np.asarray([f_positions_x(inst_positions_times),
                               f_positions_y(inst_positions_times),
                               f_positions_z(inst_positions_times)]).T

        # convert positions to Body-Fixed and scale to meters
        return self._body_j2k2bf_rotation.apply(positions)*1000
    ...
    def _sensor_velocity(self):
        ...
        inst_velocities_times = cube._inst_position_table["Times"]
        inst_velocities_times = np.linspace(inst_velocities_times[0], inst_velocities_times[-1], self.number_of_ephemerides)

        # interpolate out positions
        f_velocities_x = interp1d(cube._inst_position_table["Times"], cube._inst_position_table["Velocities"].T[0])
        f_velocities_y = interp1d(cube._inst_position_table["Times"], cube._inst_position_table["Velocities"].T[1])
        f_velocities_z = interp1d(cube._inst_position_table["Times"], cube._inst_position_table["Velocities"].T[2])

        velocties = np.asarray([f_velocities_x(inst_velocities_times),
                               f_velocities_y(inst_velocities_times),
                               f_velocities_z(inst_velocities_times)]).T

        # convert positions to Body-Fixed and scale to meters
        return self._body_j2k2bf_rotation.apply(velocties)*1000
    ...
    def _sensor_orientation(self):
        ...
        inst_pointing_times = cube._inst_pointing_table["Times"]
        inst_pointing_times = np.linspace(inst_pointing_times[0], inst_pointing_times[-1], self.number_of_quaternions)
        rotations = cube._inst_pointing_table["Rotations"]
        rotations = np.roll(np.asarray(rotations), -1, 1) # adjust rotations [0,1,2,3] -> [3,0,1,2]

        # interpolate out orientation
        orientations = Slerp(cube._inst_pointing_table["Times"], R.from_quat(rotations))(inst_pointing_times)

        return (self._body_j2k2bf_rotation*self._inst2bf_rotation.inv()*orientations).as_quat()
    ...
    def _body_j2k2bf_rotation(self):
        """
        Returns Mt*Mc where:
        Mt is the time dependant rotation from j2000 to body fixed
        Mc is contant rotation from J2000 to body fixed.
        ...
        body_rot_times = cube._body_orientation_table["Times"]
        body_const_rots = cube._body_orientation_table["ConstantRotation"]
        body_timed_rots = cube._body_orientation_table["Rotations"]
        body_timed_rots = np.roll(np.asarray(body_timed_rots), -1, 1) # adjust rotations [0,1,2,3] -> [3,0,1,2]

        interp_rot_times = np.linspace(body_rot_times[0], body_rot_times[-1], self.number_of_ephemerides)

        body_timed_rots = Slerp(body_rot_times, R.from_quat(body_timed_rots))(interp_rot_times)

        # Matrix to convert j2000 positions to body fixed
        return body_timed_rots*R.from_dcm(body_const_rots)


    @property
    def _inst2bf_rotation(self):
        """
        """
        ...
        inst_rot_times = cube._inst_pointing_table["Times"]
        inst_const_rots = cube._inst_pointing_table["ConstantRotation"]
        inst_timed_rots = cube._inst_pointing_table["Rotations"]
        inst_timed_rots = np.roll(np.asarray(inst_timed_rots), -1, 1) # adjust rotations [0,1,2,3] -> [3,0,1,2]

        interp_rot_times = np.linspace(inst_rot_times[0], inst_rot_times[-1], self.number_of_quaternions)

        inst_timed_rots = Slerp(inst_rot_times, R.from_quat(inst_timed_rots))(interp_rot_times)

        # Matrix to convert instrument pointing to j2000
        return inst_timed_rots*R.from_dcm(inst_const_rots)
