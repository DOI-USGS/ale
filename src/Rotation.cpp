...
// Helper function to convert an axis number into a unit Eigen vector down that
// axis.
 ...
 * defines AV.
 */
Eigen::Quaterniond::Matrix3 avSkewMatrix(const std::vector<double>& av) {
  if (av.size() != 3) {
    throw std::invalid_argument(
        "Angular velocity vector to rotate is the wrong size.");
  }
  Eigen::Quaterniond::Matrix3 avMat;
  avMat << 0.0, av[2], -av[1], -av[2], 0.0, av[0], av[1], -av[0], 0.0;
  return avMat;
}
...
Rotation::Rotation() : m_impl(new Impl()) {}
...
Rotation::Rotation(const std::vector<double>& angles,
...
std::vector<double> Rotation::toStateRotationMatrix(
  ...
  return {rotMat(0, 0), rotMat(0, 1), rotMat(0, 2), 0.0,          0.0,
          0.0,          rotMat(1, 0), rotMat(1, 1), rotMat(1, 2), 0.0,
          0.0,          0.0,          rotMat(2, 0), rotMat(2, 1), rotMat(2, 2),
          0.0,          0.0,          0.0,          dtMat(0, 0),  dtMat(0, 1),
          dtMat(0, 2),  rotMat(0, 0), rotMat(0, 1), rotMat(0, 2), dtMat(1, 0),
          dtMat(1, 1),  dtMat(1, 2),  rotMat(1, 0), rotMat(1, 1), rotMat(1, 2),
          dtMat(2, 0),  dtMat(2, 1),  dtMat(2, 2),  rotMat(2, 0), rotMat(2, 1),
          rotMat(2, 2)};
}
...
std::vector<double> Rotation::toEuler(const std::vector<int>& axes) const {
  ...
  if (axes[0] < 0 || axes[0] > 2 || axes[1] < 0 || axes[1] > 2 || axes[2] < 0 ||
      axes[2] > 2) {
    throw std::invalid_argument("Invalid axis number.");
  }
  Eigen::Vector3d angles =
      m_impl->quat.toRotationMatrix().eulerAngles(axes[0], axes[1], axes[2]);
...
std::vector<double> Rotation::operator()(const std::vector<double>& vector,
                                         const std::vector<double>& av) const {
  if (vector.size() == 3) {
    ...
    return std::vector<double>(rotatedVector.data(),
                               rotatedVector.data() + rotatedVector.size());
    ...
    Eigen::Vector3d rotatedVelocity =
        rotMat * velocityVector + rotationDerivative * positionVector;
...
Rotation Rotation::inverse() const {
  ...
  return Rotation(inverseQuat.w(), inverseQuat.x(), inverseQuat.y(),
                  inverseQuat.z());
}
