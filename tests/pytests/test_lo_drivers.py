...
def test_high_load(test_high_kernels):
    ...
    isd_str = ale.loads(label_file, props={'kernels': test_high_kernels, 'remove_kernels': True}, verbose=False)
...
def test_medium_load(test_medium_kernels):
    ...
    isd_str = ale.loads(label_file, props={'kernels': test_medium_kernels, 'remove_kernels': True}, verbose=True)
    isd_obj = json.loads(isd_str)
    print(json.dumps(isd_obj, indent=2))
    assert compare_dicts(isd_obj, compare_dict) == []
