...
def test_load_pds(test_iss_kernels):
    ...
    isd_str = ale.loads(label_file, props={'kernels': test_iss_kernels}, verbose=True, remove_kernels=True)
...
def test_load_isis():
    ...
    with patch('ale.base.data_isis.read_table_data', side_effect=read_detached_table):
        isd_str = ale.loads(label_file, remove_kernels=True)
    isd_obj = json.loads(isd_str)
    print(json.dumps(isd_obj, indent=2))
...
def test_load_vims_isis_naif(test_vims_kernels):
    ...
    isd_str = ale.loads(label_file, props={"kernels": test_vims_kernels}, verbose=True, remove_kernels=True)
    isd_obj = json.loads(isd_str)
    print(json.dumps(isd_obj, indent=2))
    x = compare_dicts(isd_obj, compare_dict)
    assert x == []
