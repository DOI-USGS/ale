...
def test_amica_load(test_amica_kernels):
    ...
    isd_str = ale.loads(label_file, props={'kernels': test_amica_kernels, 'remove_kernels': True})
...
def test_nirs_load(test_nirs_kernels):
    ...
    isd_str = ale.loads(label_file, props={'kernels': test_nirs_kernels, 'remove_kernels': True}, verbose=True)
    isd_obj = json.loads(isd_str)
    assert compare_dicts(isd_obj, compare_dict) == []
