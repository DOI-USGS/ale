...
class test_themisir_isis_naif(unittest.TestCase):
    ...
    def test_start_time(self):
        with patch.object(ale.drivers.ody_drivers.NaifSpice, 'ephemeris_start_time', new_callable=PropertyMock) as ephemeris_start_time:
    ...
    def test_ephemeris_start_time(self):
        with patch.object(OdyThemisIrIsisLabelNaifSpiceDriver, 'band_times', new_callable=PropertyMock) as band_times:
            band_times.return_value = [400001111.2222333, 400001118.9115534]
            assert self.driver.ephemeris_start_time == 400001111.2222333
    ...
    def test_band_times(self):
        with patch.object(OdyThemisIrIsisLabelNaifSpiceDriver, 'start_time', new_callable=PropertyMock) as start_time:
            start_time.return_value = 400001111.2222333
...
class test_themisvis_isis_naif(unittest.TestCase):
    ...
    def test_start_time(self):
        with patch('ale.drivers.ody_drivers.pyspiceql.strSclkToEt', return_value=[0]) as strSclkToEt:
            self.driver.label["IsisCube"]["Instrument"]["SpacecraftClockOffset"] = 10
            assert self.driver.start_time == 9.9976
            strSclkToEt.assert_called_with(frameCode=-53, sclk='1023406812.230', searchKernels=False, useWeb=False)
            assert strSclkToEt.call_count == 1

    def test_ephemeris_start_time(self):
        with patch.object(OdyThemisVisIsisLabelNaifSpiceDriver, 'band_times', new_callable=PropertyMock) as band_times:
            band_times.return_value = [392211098.2259215]
            assert self.driver.ephemeris_start_time == 392211098.2259215

    def test_ephemeris_stop_time(self):
        with patch.object(OdyThemisVisIsisLabelNaifSpiceDriver, 'band_times', new_callable=PropertyMock) as band_times:
            band_times.return_value = [392211098.2259215]
            print(self.driver.num_frames, self.driver.interframe_delay)
    ...
    def test_band_times(self):
        with patch.object(OdyThemisVisIsisLabelNaifSpiceDriver, 'start_time', new_callable=PropertyMock) as start_time:
            start_time.return_value = 392211096.42832154
