...
def test_fc_load(fc_kernels, label_type):
    ...
    isd_str = ale.loads(label_file, props={'kernels': fc_kernels, 'remove_kernels': True})
...
def test_vir_load(vir_kernels):
    ...
    with patch('ale.drivers.dawn_drivers.read_table_data', return_value=12345) as read_table_data, \
        ...
        isd_str = ale.loads(label_file, props={"kernels": vir_kernels, "nadir": False, 'remove_kernels': True}, verbose=False)
        print(isd_str)
        isd_obj = json.loads(isd_str)
...
class test_dawn_fc_pds3_naif(unittest.TestCase):
    ...
    def test_ephemeris_start_time(self):
        with patch('ale.spiceql_access.spiceql_call', side_effect=[54321, 12345]) as spiceql_call:
            ...
            calls = [call('translateNameToCode', {'frame': 'DAWN', 'mission': 'fc2', 'searchKernels': False}, False),
